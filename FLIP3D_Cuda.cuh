...
#define FOR_NEIGHBOR(n) for( int dx=-n; dx<=n; dx++ ) for( int dy=-n; dy<=n; dy++ ) for( int dz=-n; dz<=n; dz++ ) {
#define END_FOR }
...
__device__ __host__
REAL inline trilinearUnitKernel(REAL r) {
	r = abs(r);
	if (r > 1) return 0;
	return 1 - r;
}
...
__device__ __host__
REAL inline trilinearHatKernel(REAL3 r, REAL support) {
...
__global__ void ResetCell_D(VolumeCollection volumes, uint gridRes) {
	...
	volumes.hasVelocity.writeSurface<uint4>(make_uint4(0, 0, 0, 0), x, y, z);
	volumes.velocity.writeSurface<REAL4>(make_REAL4(0, 0, 0, 0), x, y, z);
	volumes.newVelocity.writeSurface<REAL4>(make_REAL4(0, 0, 0, 0), x, y, z);
}

__global__  void transferToCellAccumPhase(VolumeCollection volumes, REAL3* pos, REAL3* vel, uint* gridHash, uint* gridIdx, uint* cellStart, uint* cellEnd, uint numParticle, uint gridRes) {
	...
	uint cellCount = (gridRes) * (gridRes) * (gridRes);
	REAL cellSize = 1.0 / gridRes;

	REAL3 xVelocityPos = make_REAL3(x, (y + 0.5), (z + 0.5)) * cellSize;
	REAL3 yVelocityPos = make_REAL3((x + 0.5), y, (z + 0.5)) * cellSize;
	REAL3 zVelocityPos = make_REAL3((x + 0.5), y + 0.5, z) * cellSize;

	REAL4 thisVelocity = make_REAL4(0, 0, 0, 0);
	REAL4 weight = make_REAL4(0, 0, 0, 0);
	...
	FOR_NEIGHBOR(1) {
		...
		uint cell = (x + dx) * gridRes * gridRes + (y + dy) * gridRes + z + dz;
		...
		if (cell >= 0 && cell < cellCount) {
			if (startIdx != 0xffffffff)
			{
				...
				{
					uint sortedIdx = gridIdx[i];
					if (sortedIdx >= 0 && sortedIdx < numParticle) {

						REAL3 pPosition = pos[sortedIdx];
						REAL3 pVelocity = vel[sortedIdx];
						REAL thisWeightX = trilinearHatKernel(pPosition - xVelocityPos, cellSize);
						REAL thisWeightY = trilinearHatKernel(pPosition - yVelocityPos, cellSize);
						REAL thisWeightZ = trilinearHatKernel(pPosition - zVelocityPos, cellSize);
	...
	volumes.velocityAccumWeight.writeSurface<REAL4>(weight, x, y, z);
	volumes.velocity.writeSurface<REAL4>(thisVelocity, x, y, z);
}

__global__  void transferToCellDividePhase(VolumeCollection volumes, REAL3* pos, uint* gridHash, uint* gridIdx, uint* cellStart, uint* cellEnd, uint numParticle, uint gridRes) {
	...
	REAL cellSize = 1.0 / gridRes;
	REAL4 weight = volumes.velocityAccumWeight.readSurface<REAL4>(x, y, z);
	uint4 hasVelocity = volumes.hasVelocity.readSurface<uint4>(x, y, z);
	REAL4 velocity = volumes.velocity.readSurface<REAL4>(x, y, z);
	...
	volumes.velocity.writeSurface<REAL4>(velocity, x, y, z);
	volumes.newVelocity.writeSurface<REAL4>(velocity, x, y, z);
	volumes.hasVelocity.writeSurface<uint4>(hasVelocity, x, y, z);
	...
	if (startIdx != 0xffffffff)
	{
		...
		for (uint i = startIdx; i < endIdx; i++)
		{
			...
			volumes.content.writeSurface<uint>(CONTENT_FLUID, x, y, z);
			break;
		}
	}
}
...
__global__  void transferVolumeFractionsToCell(VolumeCollection volumes, REAL3* pos, REAL4* volumeFraction, uint* gridHash, uint* gridIdx, uint* cellStart, uint* cellEnd, uint numParticle, uint gridRes, float4 phaseDensities) {
	...
	float3 centerPos = make_float3(x + 0.5, y + 0.5, z + 0.5) * cellSize;
	...
	if (startIdx != 0xffffffff)
	{
		...
		for (uint i = startIdx; i < endIdx; i++)
		{
			...
			REAL3 pPosition = pos[sortedIdx];
			REAL thisWeight = trilinearHatKernel(pPosition - centerPos, cellSize);

			fractions += thisWeight * volumeFraction[sortedIdx];
			weight += thisWeight;
		}
	}
