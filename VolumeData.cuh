#ifndef __VOLUMEDATA_CUH__
#define __VOLUMEDATA_CUH__
...
VolumeData createField3D(int sizeX, int sizeY, int sizeZ, dim3 cudaGridSize, dim3 cudaBlockSize, T initialValue, bool filter, cudaTextureAddressMode addressMode = cudaAddressModeClamp) {
	...
	cudaGetErrorString(cudaMalloc3DArray(&result.array, &channelDesc, extent, cudaArraySurfaceLoadStore));
	...
	cudaGetErrorString(cudaCreateTextureObject(&result.texture, &resDesc, &texDesc, nullptr));

	cudaGetErrorString(cudaCreateSurfaceObject(&result.surface, &resDesc));
...
inline void releaseField3D(VolumeData volume) {
	cudaGetErrorString(cudaFreeArray(volume.array));
	cudaGetErrorString(cudaDestroySurfaceObject(volume.surface));
	cudaGetErrorString(cudaDestroyTextureObject(volume.texture));
}
