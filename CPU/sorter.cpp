...
Sorter::Sorter( int gn ) {
	cells = alloc3D<vector<particle *>>(gn,gn,gn); //gn*gn*gn 만큼의 cell 생성
	this->gn = gn;
}
...
void Sorter::sort( std::vector<particle *> &particles ) { //cell에 particle 넣기
	// Clear All Cells
	FOR_EVERY_CELL(gn) {
		cells[i][j][k].clear();
	} END_FOR
...
std::vector<particle *> Sorter::getNeigboringParticles_cell( int i, int j, int k, int w, int h, int d ) { //ijk -> cell index, whd -> neighbor range
	std::vector<particle *> res;
...
int	 Sorter::getNumFluidParticleAt(int i, int j, int k) {
	int cnt = 0;
...
FLOAT Sorter::levelset( int i, int j, int k, FLOAT ***halfwall, FLOAT density ) {
	...
	for( int a=0; a<cells[i][j][k].size(); a++ ) { 
		if( cells[i][j][k][a]->type == FLUID ) {
			accm += cells[i][j][k][a]->dens; //cell 안의 particle들의 밀도
		} else {
			return 1.0;
		}
	}
